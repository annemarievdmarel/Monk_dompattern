...
- use crowds and displacements combined? low (crowd) vs high (displace) aggresssion
- use aggression away from feeder or all aggression? --> can use all aggression (see framework)
- remove duplicates by behavior or combined? 
...
## raw data
behave_agg <- read.csv("output/2021_interactions_bin0-12.csv") %>%
  dplyr::select(-X)
# check
unique(behave_agg$actor)
unique(behave_agg$subject)
unique(behave_agg$date)


# period file
#obs.day.key <- read.csv("../2021_timeline_fieldwork.csv") 
bin.dates <- read.csv("2021_3daybins.csv") #%>%
unique(bin.dates$bin)

# bird list
birdIDs_capture<- read.csv("2021_birdIDs.csv") 
...
dyad.totalaggXbin <-read.csv("./output/dyad.aggXbin.bin0-12.csv")
...
# observation hours:
hours_obs_raw <- read.csv("2021_observation hours_cleaned.csv") %>%
  dplyr::select(-X)

bins <- read.csv("../field season 2021/2021_3daybins.csv") %>%
  dplyr::select(date,  rank_assessment) %>%
  drop_na() %>%
  rename(bin=rank_assessment) %>% 
  group_by(bin) %>%
  mutate(ndays=n_distinct(date)) 
...
sum.aggDCXbin_conservative <-dyad.totalaggXbin %>%
  group_by(bin) %>%
  summarize(n.agg = sum(n.aggXbin), n.dyads=length(actor), birds=n_distinct(actor))
...
write.csv(dyad.totalaggXbin, "./output/dyad.aggXbin.bin0-12.csv")
...
#strategiesDC_20birds <- strategies
#strategies_removal1 <- strategies
#strategies_removal2 <- strategies
#strategies_removal3 <- strategies

strategies_longperturb <- bind_rows(strategiesDC_20birds, 
                                    strategies_removal1,
                                    strategies_removal2,
                                    strategies_removal3)
strategies_longperturb <- strategies_longperturb %>%
  rename(bin=run.code) %>%
  arrange(bin)
...
write.csv(strategies_longperturb, "output/dompatterns_longperturbations.csv")
```
...
strategies <- read.csv("output/dompatterns_longperturbations.csv") %>%
...
hoursobs <- hours_obs_raw %>%
  left_join(bins, by="date") %>%
  filter(bin!="NA", bin!="13")

## observation hours
# duration of observations / bin
hours_lunchincl <- hoursobs %>% 
  filter(type=="obs") %>%
  group_by(bin) %>%
  summarise(total=sum(totaltime_hour))

# lunch breaks --> no observations performed
lunchbreaks <- hoursobs %>% 
  filter(subject=="lunch") %>%
  group_by(bin) %>%
  summarise(lunch=sum(totaltime_hour))

# Total hours observed by bin
total_hoursXbin <- left_join(hours_lunchincl, lunchbreaks) %>%
  mutate(hours_obs = total-lunch)

sum(total_hoursXbin$hours_obs)

# summ total hours X bin
summ_hoursXbin <- total_hoursXbin %>%
  summarize(mean=mean(hours_obs),
            sd=sd(hours_obs))
...
# Total hours observed by bin
person_hoursXbin <- left_join(sumperson, breaks) %>%
  mutate(person_hours = sumperson-breaks)

sum(person_hoursXbin$person_hours)

## combine
obs.hours <- total_hoursXbin %>% 
  left_join(person_hoursXbin) %>%
  dplyr::select(bin, hours_obs, person_hours ) 


```
277 hours and 1023 person hours across 39.5 days, with an average of 21.3 ± 2.2 (SD) hours of observation per three-day bin 
...
summ_ag_all <- sum.aggDCXbin_conservative %>%
  ...
  mutate(ag_bird= n.agg/birds, 
         ag_obs= n.agg/birds/hours_obs, 
         ag_person_obs = n.agg/birds/person_hours, 
         mean = mean(ag_obs),
         sd = sd(ag_obs))
...
```{r}
plot_aggression_control <-  ggplot(summ_ag_all, aes(x = bin)) +
  
  # aggression / bird / person hours observed
  geom_line(aes(y=ag_person_obs)) + 
  geom_point(aes(y=ag_person_obs)) +
  ...
  labs(y="rate of aggression") + #, title = "Amount of aggression controlled for group size and hours observed"
  
  geom_vline(xintercept = 1.5, linetype="dashed", color =  alpha("black", 0.6)) + # removal
  geom_vline(xintercept = 3.5, linetype="solid", color =  alpha("black", 0.6)) + # reintroduction
  geom_vline(xintercept = 5.5, linetype="dashed", color =  alpha("black", 0.6)) +
  geom_vline(xintercept = 7.5, linetype="solid", color =  alpha("black", 0.6)) +
  geom_vline(xintercept = 9.5, linetype="dashed", color =  alpha("black", 0.6)) +
  geom_vline(xintercept = 11.5, linetype="solid", color =  alpha("black", 0.6)) +
  
  # add average rate of aggression
  geom_hline(yintercept = ave.ag.rate, color = alpha("grey", 0.6), size=1)
 

plot_aggression_control
```
...
plot_strategies_summ <- ggplot(strategies, 
                               aes(bin, dominance_pattern)) +
  geom_point(size = 5, shape = 21, colour = "black", aes(fill = dominance_pattern )) +
...
randomization by trial instead of combined
...
### ggraph with y-axis (20 birds)
somehow only one bin works at a time, not the loop
...
p8 <-
 ...
 #ggsave(paste0("./figures/aggnwXbin_CBreport_bin",run.code,"_3x3.pdf"), 
#        width = 3, height = 3,
#       device = cairo_pdf,
#       plot=p) #device = cairo_pdf,

# ggsave(paste0("./figures/aggnwXbin_CBreport_bin",run.code,".pdf"), 
#       device = cairo_pdf,
#       plot=p) 
#dev.off()
...
p11 <-
 ...
 ggsave(paste0("./figures/aggnwXbin_CBreport_bin",run.code,"_3.5x3.pdf"), 
        width = 3.5, height = 3,
       device = cairo_pdf,
       plot=p) #device = cairo_pdf,
...
# save plots
nw_after_removal <- ggarrange(p2, p6, p10,  
                              nrow = 3, ncol = 1)
nw_after_removal
ggsave( "./figures/nw_after_removal.pdf",
        width = 3.5, height = 9,
        device = cairo_pdf)

nw_before_reintro <- ggarrange(p3, p7, p11, 
                               nrow = 3, ncol = 1)
nw_before_reintro 
ggsave( "./figures/nw_before_reintro.pdf",
        width = 3.5, height = 9,
        device = cairo_pdf)
...
### ggraph loop with y-axis
somehow only one bin works at a time, not the loop

```{r loop aggression network}
...
## loop to get aggression network
start.time <- Sys.time()
...
# SUBSET data by bin
 ...
 # total aggressive events
 total_agg <- sum(bin.data$weight)

 # percent rule breakers and followers of total dyads
 rulebreakers <- length(bin.data$rule[bin.data$rule=="breaker"])/length(bin.data$actor) *100
 rulefollowers <- length(bin.data$rule[bin.data$rule=="follower"])/length(bin.data$actor) *100
 ...
 #range_ec_GPG <- range(ec_rank$dom_ec)
  ...
  ## change x axis to provide jitter of nodes
  #jitter<- c(0,-4,-5,1,2,3,0.5,3.5,-1.5,-3,-4,5,1.5,-2,2.5,5,-3.5,-1,-2.5,4.5,4)
  jitter1<- c(0,2,0.5,3,5,4,2,3,4,-4,-1,-3,-4,-5,0,-3,-2,5,1,-2) # diamond shape
  ec_lay <- cbind(jitter1, ec_lay)
  ec_lay<-ec_lay[,-2] # delete column 2
  #colnames(ec_lay) <- c("V1", "V2")
...
## add vertex/node attribute
V(gdf)$name <- attrib_node$id
V(gdf)$dom <- power_plot$power # now we have to reverse y-axis
V(gdf)$sex <- attrib_node$sex

## set node color  
# all green when during removal period focals
#V(gdf)$fill <- c(alpha("#b8d79c", 1)) # all birds green

# highlight focal; "#1f78b4" for BBB; "#33a02c" for GPG; "#ff7f00" for OPP
#V(gdf)$fill<- c( alpha("#b8d79c", 1), "#1f78b4")[1+(V(gdf)$name=="BBB")]
#V(gdf)$fill<- c(alpha("#b8d79c", 1), "#6a51a3")[1+(V(gdf)$name=="GPG")]  # "#6a51a3" -> PURPLE & "#33a02c" -> DARK GREEN
V(gdf)$fill<- c("#ff7f00", alpha("#b8d79c", 0.8))[1+(V(gdf)$name=="OPP")] # "#ff7f00" -> orange & 
...
set.seed(42)
#title<-unique(attrib_edge$type[attrib_edge$bin==run.code])
...
# plot
#for(i in seq_along(unique(binBBB))){
...
p1 <-
  ...
  # highlight focal; "#1f78b4" for BBB; green("#33a02c") or purple (alpha("#7030A0", 0.6) for GPG; "#ff7f00" for OPP
  scale_fill_manual(name = "fill",
                     values=  c( "#ff7f00", alpha("#b8d79c", 1))) + #c(  "#1f78b4", alpha("#b8d79c", 1)) / alpha("#b8d79c", 1))
  scale_color_manual(values="white") +
  scale_shape_manual( name = "sex", 
                      values = c( 21, 23 ), #c( 21, 23 )  19, 17 
                      labels = c( "female", "male" )) +
...
ggplot(summ_rand_removalXrun_all, aes(x = prop, y = change, fill = change)) +
  #stat_density_ridges(quantile_lines = TRUE, quantiles = c(0.025, 0.975), alpha = 0.7) +
    #stat_density_ridges(quantile_lines = TRUE, quantiles = c(0.025, 0.975), alpha = 0.7)
  #geom_density_ridges2() 
  # geom_density_ridges(
  ...
  #)
  #stat_density_ridges(quantile_lines = TRUE, quantiles = c(0.025, 0.975), alpha = 0.7) +
  geom_density_ridges( stat="binline", bins=50, scale=0.8, draw_baseline = F) + #alpha=0.6,
  ...
  ##DH-DH
  geom_segment(y=4, yend=4.8, x = 0, xend=0, size=1,  linetype = "dashed")  +
  geom_segment(y=4, yend=4.8, x = 0.75, xend=0.75, size=1,  linetype = "dashed")  +
  #DH-B
  geom_segment(y=3, yend=3.8, x = 0, xend=0, size=1,  linetype = "dashed")  +
  geom_segment(y=3, yend=3.8, x = 1.0, xend=1.0, size=1,  linetype = "dashed")  +
  #B-DH
  geom_segment(y=2, yend=2.8, x = 0, xend=0, size=1,  linetype = "dashed") +
  geom_segment(y=2, yend=2.8, x = 0.75, xend=0.75, size=1,  linetype = "dashed") +
  #B-B
  geom_segment(y=1, yend=1.8, x = 0, xend=0, size=1,  linetype = "dashed") +
  geom_segment(y=1, yend=1.8, x = 0.75, xend=0.75, size=1,  linetype = "dashed") 
  
 

DH.ridge <- ggplot(rand.pattsXorder.ord, aes(x=nrand.DH, y=ordtree.horiz.plotord, group = order)) +
  geom_density_ridges(stat = "binline", binwidth = 1, scale = 0.85, color="black", fill="black", alpha=0.4) +
  theme_ridges(font_size = 12,
    font_family = "",
    line_size = 0.5,
    grid = TRUE,
  ...
  geom_segment(data=observed.patternsXorder.ord, aes(x=obs.DH, y=ordtree.horiz.plotord-0.05, xend=obs.DH, yend=ordtree.horiz.plotord+0.75), color='black', alpha=0.9, size=1.5)

```

 experimental timeline

```{r plot experimental setup}
# empty plot to fill in inkscape
plot_setup <- ggplot(summary, aes(x=rank_assessment, y=obs_days)) +
  ...
  # inlcude lines of removal and reintroduction periods
  geom_vline(xintercept = 1.5, linetype="dashed", color =  alpha("black", 0.6)) + # removal
  geom_vline(xintercept = 3.5, linetype="solid", color =  alpha("black", 0.6)) + # reintroduction
  geom_vline(xintercept = 5.5, linetype="dashed", color =  alpha("black", 0.6)) +
  geom_vline(xintercept = 7.5, linetype="solid", color =  alpha("black", 0.6)) +
...
```
 ...
 randomization totalled, instead of per trial separate and then averaging
```{r dom pattern randomizations_removal}
...
## before removal vs after removal ----
# observed pattern change from downward heuristic to downward heuristic for trial 1 and 3 (bin 11->12,  bin 19->20), from bullying to bullying for trial 2 (bin 15 ->bin16) -->  observed proportion remained the same

binchange_removal <- c(11, 15, 19)

rand_summ_removal <- before_after %>%
  filter(bin %in% binchange_removal) %>%
  group_by(random_pattern_change) %>%
  summarise(random_n=length(random_pattern_change), random_prop = random_n/3000) %>%  # 3 trials with 1000 randomized values
  rename(change=random_pattern_change) %>%
  dplyr::select(-random_n) %>%
  mutate(prop=round(random_prop, 2)) %>%
  dplyr::select(-random_prop)
...
# set line widtth
curves <- matrix(nrow=ncol(rand_summ_removal),ncol=ncol(rand_summ_removal),0)
curves[1,1]<- (r.removal.mx[1,1]*10)+1       
curves[1,2]<- (r.removal.mx[1,2]*10)+1 
curves[2,2]<- (r.removal.mx[2,2]*10)+1  
curves[2,1]<- (r.removal.mx[2,1]*10)+1 
...
# plot transition diagram  
pdf("./figures/plot_random_removal.pdf")  # open and save the pdf file
plotmat(r.removal.mx, 
        #pty="s",
        #pos=c(1,1),
...
# plot transition matrix
pdf("./figures/plot_obs_removal.pdf")  # open and save the pdf file
plotmat(obs.removal.mx, 
        #pty="s",
        #pos=c(1,1),
        ...
        # self 
         relsize=0.7, self.lwd=c(4.3,7.7), #self.lwd = linewidth, 
        self.shiftx=c(.1, -0.1), self.shifty=c(-0.1, 0.1), 
...
rand_summ_reintro <- before_after %>%
  filter(bin %in% binchange_reintro) %>%
  group_by(random_pattern_change) %>%
  summarise(n=length(random_pattern_change), random_prop = n/3000) %>% # 3 trials with 1000 randomized values
  rename(change=random_pattern_change) %>%
  dplyr::select(-n) %>%
  mutate(prop=round(random_prop, 2)) %>%
  dplyr::select(-random_prop)
...
# set line widtth
r.lwd.reintro <- matrix(nrow=ncol(rand_summ_reintro),ncol=ncol(rand_summ_reintro),0)
r.lwd.reintro[1,1]<- (r.reintro.mx[1,1]*10)+1       
r.lwd.reintro[1,2]<- (r.reintro.mx[1,2]*10)+1 
r.lwd.reintro[2,2]<- (r.reintro.mx[2,2]*10)+1  
r.lwd.reintro[2,1]<- (r.reintro.mx[2,1]*10)+1 
...
# transition diagram
pdf("./figures/plot_random_reintro.pdf")  # open and save the pdf file
plotmat(r.reintro.mx, 
        #pty="s",
        #pos=c(1,1),
...
distribution per trial instead of per run
...
#bb
randbb <- rand_removalXrun_all %>%
  filter(change == "B-B")

summ_randbb <- randbb %>%
  summarize(a = mean(random_prop), 
            s = sd(random_prop),
            n = n(),
...
plot_remove_bb <- ggplot(randbb, aes(x=random_prop)) +
  geom_bar() +
  theme_classic() +
  scale_x_continuous(limits = c(0,1)) +
  # confidence interval
  geom_vline(xintercept = summ_randbb$left, linetype = "dotted" ) +
  geom_vline(xintercept = summ_randbb$right, linetype = "dotted" ) +
  # observed proportion
  geom_vline(xintercept = 0.33, linetype = "dashed", color="red") +
  labs(x="transition proportion", title="B - B")


randbbplot <- rand_summ_removal %>%
  filter(change == "B-B")
summary(randbbplot$prop)

plot_remove_bb <- ggplot(randbbplot, aes(x=prop)) +
  geom_bar() +
  theme_classic() +
  scale_x_continuous(limits = c(0,1)) +
  # confidence interval
  geom_vline(xintercept = left, linetype = "dotted" ) +
  geom_vline(xintercept = right, linetype = "dotted" ) +
  # observed proportion
  geom_vline(xintercept = 0.33, linetype = "dashed", color="red") +
  labs(x="transition proportion", title="B - B")

# b-dh
randbdh <- rand_removal %>%
  filter(change == "B-DH")
a <- randbdh$aveprop
s <- randbdh$sdprop
n <- randbdh$n
...
randbdhplot <- rand_summ_removal %>%
  filter(change == "B-DH")
summary(randbbplot$prop)

plot_remove_bdh <- ggplot(randbdhplot, aes(x=prop)) +
  geom_bar() +
  theme_classic() +
  scale_x_continuous(limits = c(0,1)) +
  # confidence interval
  ...
  # observed proportion
  geom_vline(xintercept = 0.33, linetype = "dashed", color="red") +
  labs(x="transition proportion",title="B - DH")

#dh-dh
randdhdh <- rand_removal %>%
  filter(change == "DH-DH")
a <- randdhdh$aveprop
s <- randdhdh$sdprop
n <- randdhdh$n
...
randdhdhplot <- rand_summ_removal %>%
  filter(change == "DH-DH")
summary(randdhdhplot$prop)

plot_remove_dhdh <- ggplot(randdhdhplot, aes(x=prop)) +
  geom_bar() +
  theme_classic() +
  scale_x_continuous(limits = c(0,1)) +
  # confidence interval
  ...
  # observed proportion
  geom_vline(xintercept = 0.33, linetype = "dashed", color="red") +
  labs(x="transition proportion", title="DH - DH")

#dh-b
randdhb <- rand_removal %>%
  filter(change == "DH-B")
a <- randdhb$aveprop
s <- randdhb$sdprop
n <- randdhb$n
...
randdhbplot <- rand_summ_removal %>%
  filter(change == "DH-B")
summary(randdhbplot$prop)

plot_remove_dhb <- ggplot(randdhbplot, aes(x=prop)) +
  geom_bar() +
  theme_classic() +
  scale_x_continuous(limits = c(0,1)) +
  # confidence interval
  ...
  # observed proportion
  geom_vline(xintercept = 0.33, linetype = "dashed", color="red") +
  labs(x="transition proportion", title="DH - B")


# combine plots

trans_prop_removal <- ggarrange(plot_remove_dhdh,
          plot_remove_dhb,
          plot_remove_bdh,
          plot_remove_bb,
          nrow = 2, ncol = 2)

ggsave("output/Fig. trans prop removal.pdf", height = )
