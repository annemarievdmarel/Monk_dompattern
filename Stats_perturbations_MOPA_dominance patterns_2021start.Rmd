...
aggDC <- behave_agg %>% 
  filter(behavior=="displace" | behavior=="crowd", 
         bin %in% c(7, 8,9,10) # c(7, 8,9,10) = 20 birds
         #bin<7   # bin<7 = 22 birds, 
         ) %>%
...
summ_ag_21start <- sum.aggDCXbin_conservative %>%
...
plot_controlaggression_21start<-  summ_ag_21start %>%
  ...
  # aggression controlled for group size and person hours observed
...
strategies <- read.csv("output/summ_agg2021.csv")
...
```{r}
  ...
  ggsave("./figures/plot_Fig2021_start.pdf", width= 175, height=125, units="mm")
