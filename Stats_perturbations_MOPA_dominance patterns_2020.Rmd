...
obshoursday$min <-  hm(obshoursday$min)
obshoursday$max <-  hm(obshoursday$max)

obshoursday <- obshoursday %>%
   mutate(totaltime=as.duration(max-min), # calculate total time
...
ave.ag.rate <- mean(summ_ag_all$ag_bird_obs)
sd.ag.rate <- sd(summ_ag_all$ag_bird_obs)
...
```{r}
  ...
  ggarrange(plot_aggression_control2020 , plot_strategies_2020, 
            nrow=2, ncol=1, align = "hv",
            labels = c("(a)", "(b)")
            ) # legend = "bottom", label.x = 0.15, label.y=0.85

  ggsave("./figures/plot_Fig2020_narrow.pdf", width= 100, height=125, units="mm")
