...
## Fig 4a) group 1: aggression rate controlled for person hours observed and group size
...
obs_hours1$start <-  hm(obs_hours1$start)
obs_hours1$end <-  hm(obs_hours1$end)

## total observation hours
hoursobs1 <- obs_hours1 %>%
  left_join(bin_dates %>%
              filter(bin<27), by="date") %>%
  mutate(totaltime=as.duration(end-start), # calculate total time
...
plot_aggression_22group1 <-  summ_ag_group1 %>%
 ...
 # add average rate of aggression
  ...
  scale_y_continuous(limits=c(0.0, 12.5)) +
...
## Fig 5a) group 2 aggression rate
...
## lubridate: hm(..., quiet = FALSE, roll = FALSE)
obs_hours2$start <-  hm(obs_hours2$start)
obs_hours2$end <-  hm(obs_hours2$end)
...
### Fig 4b) group 1 dominance patterns
...
### Fig 5b) group 2 dominance patterns
...
```{r}
  ...
  ggsave("./figures/plot_Fig2022_group1.pdf", width= 250, height=125, units="mm")
...
```{r}
  ...
  ggsave("./figures/plot_Fig2022_group2.pdf", width= 250, height=125, units="mm")
         #width= 7, height=7, units="in") # full page  width= 7, height=11, units="in") # half page width= 125, height=250, units="mm"
```
...
```{r}
  ...
  ggsave("./figures/plot_Fig2022.pdf", width= 250, height=250, units="mm")
```
...
obs_pattern_change <- dompatterns %>%
...
## Transition proportion X focal rank
...
mx <- reshape2::dcast(obs_top, before~after, value.var="prop") 
...
linewidth <- matrix(nrow=3,
                    ncol=3,0)
linewidth[1,1]<- (obs.mx[1,1]*10)+1      
linewidth[1,2]<- (obs.mx[1,2]*10) 
linewidth[1,3]<- (obs.mx[1,3]*10)+1 
linewidth[2,2]<- (obs.mx[2,2]*10)+1  
linewidth[2,1]<- (obs.mx[2,1]*10)+1
linewidth[2,3]<- (obs.mx[2,3]*10)+1  
linewidth[3,1]<- (obs.mx[3,1]*10)+1       
linewidth[3,2]<- (obs.mx[3,2]*10) 
linewidth[3,3]<- (obs.mx[3,3]*10)+1 
...
obs_ml$after <- factor(obs_ml$after, 
                  #levels=c("bullying", "close competitor", "downward heuristic"), 
                                levels=c( "B" , "CC","DH"))
# create matrix
mx <- reshape2::dcast(obs_ml, before~after, value.var="prop") 
...
linewidth[1,3]<- (obs.mx[1,3]*10)+1 
linewidth[2,2]<- (obs.mx[2,2]*10)+1  
linewidth[2,1]<- (obs.mx[2,1]*10)
linewidth[2,3]<- (obs.mx[2,3]*10)+1  
linewidth[3,1]<- (obs.mx[3,1]*10)      
linewidth[3,2]<- (obs.mx[3,2]*10)
linewidth[3,3]<- (obs.mx[3,3]*10)+1 
